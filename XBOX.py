...
def banner():
	...
	print(f'\t       FACEBOOK METAVERSE  ')
	print(f'\t  Author <•> SHOLEH-XDy  & ARW LEDA ')
...
def dump_massal():
	...
	try:
		...
		print('<•> Masukkan Angka Sayang, jangan Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		print('')
		print(f'<•> ID SIAP MENDESAH {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}<•> 1. Akun KAKEK NENEK ')
	print('<•> 2. Akun REMAJA ')
	print('<•> 3. JANDA BRUTAL ')
	hu = input('<•> Pilih : ')
	if hu in ['1','01']:
...
#>>>>> THANKS TO THIS HERE <<<<<<<#
#>>>>> Alvino_Adijaya_Xy <<<<<##-----------------[ IMPORT-MODULE ]-------------------#
import requests,bs4,json,os,sys,random,datetime,time,re
import urllib3,rich,base64
from rich.table import Table as me
from rich.console import Console as sol
