...
try:
	...
	print(Panel(f"""{P2}connection problem, please check your connection again""",width=80,style=f"{color_table}"))
	sys.exit()
...
class instagram:
    ...
    def logo(self):
        for i in external:
            ...
            banner()
            oi = '\t              [green]%s'%(nama)
            cetak(nel(oi, title='LOGIN SUCCESSFUL AS '))
...
# PERANG NYA SERU
    def crackAPI(self,user,pas):
        ...
        animasi = random.choice(["[KLEIN]","[XNX]"])
...
#					"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(1000000000, 99999999999),pw),
                ...
                if "userId" in str(x_jason):
                    ...
                    print(f"\r{B}[{P}✔{B}]{P} Status :{H} ALIVE-XNX-KLEIN{C}                    ")
              ...
              #      print(f"\r{H}[{R}✘{H}]{H} Status :{R} Check Point{R}                    ")
                    print(f"\r{B}[{P}✘{B}]{P} Status :{K} CP-XNX-KLEIN{C}                    ")
...
# PILIHAN MENU
    def menu(self):
        ...
        if c=='':
            ...
            print('')
            mas=input(f'>> Enter y to continue  y/t{C} ')
            if mas in ['y','Y']:
            	massal(self)
            elif mas in ['t','T']:
                exit()
...
if __name__=='__main__':
    try:
        os.system('git pull')
        os.system('curl -l https://raw.githubusercontent.com//IGD/main/ua.txt > ua.txt')
        #mengecek_()
        login_kamu()
    except requests.exceptions.ConnectionError:
        exit(f'\n [{M}!{C}] INTERNET CONNECTION ERROR')
